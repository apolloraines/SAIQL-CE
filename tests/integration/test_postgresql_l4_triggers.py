#!/usr/bin/env python3
"""
SAIQL PostgreSQL L4 Harness Tests - Triggers

Validates PostgreSQL adapter L4 capabilities per collab rules:
- C1) Subset definition: allowlist + denylist documented and enforced
- C2) Extraction: Enumerate triggers in subset (table, timing, event, body, dependencies)
- C3) Emission: Create triggers after dependent routines exist
- C4) Validation (behavioral): Seed rows, apply DML, validate trigger effects
- C5) Limitations: Skipped triggers listed with reason and count

Proof-first approach per Apollo rules. Must pass 3x from clean state.

L4 Subset Rules (documented per C1):
- ALLOWLIST:
  - Level: ROW-level triggers
  - Timing: BEFORE, AFTER
- DENYLIST:
  - Level: STATEMENT-level triggers (not portable)
  - Timing: INSTEAD OF triggers (view-specific)

Note: Trigger functions are typically VOLATILE because they modify NEW row.
This is expected behavior and does NOT exclude them from safe subset.

Evidence:
- PostgreSQL 15 container on port 5433
- Fixture: /mnt/storage/DockerTests/postgresql/fixtures/04_triggers.sql
- Expected triggers:
  - Safe: trg_employee_update_timestamp, trg_salary_validate, trg_dept_code_uppercase (3)
  - Skipped: trg_unsafe_statement (statement-level) (1)
"""

import pytest
import logging
from typing import Dict, Any, List

logger = logging.getLogger(__name__)

# Expected safe triggers
SAFE_TRIGGERS = [
    'trg_employee_update_timestamp',
    'trg_salary_validate',
    'trg_dept_code_uppercase'
]

# Expected skipped triggers with reasons
SKIPPED_TRIGGERS = {
    'trg_unsafe_statement': 'statement-level trigger'
}


class TestPostgreSQLL4SubsetDefinition:
    """L4 Harness: C1 - Subset definition tests"""

    @pytest.fixture(scope='class')
    def pg_adapter(self):
        """PostgreSQL adapter fixture"""
        from extensions.plugins.postgresql_adapter import PostgreSQLAdapter

        adapter = PostgreSQLAdapter(
            host='localhost',
            port=5433,
            database='saiql_phase11_test',
            user='saiql_user',
            password='SaiqlTestPass123'
        )
        yield adapter
        adapter.close()

    def test_l4_c1_subset_rules_enforced(self, pg_adapter):
        """
        C1.1: Subset rules enforcement

        Requirement: get_safe_triggers() only returns triggers matching allowlist
        """
        print("\n=== C1.1: Subset rules enforcement ===")

        safe_triggers = pg_adapter.get_safe_triggers('public')
        safe_names = [t['name'] for t in safe_triggers]

        print(f"  Safe triggers found: {len(safe_triggers)}")
        for name in safe_names:
            print(f"    ✓ {name}")

        # All expected safe triggers should be present
        for expected in SAFE_TRIGGERS:
            assert expected in safe_names, f"Missing expected safe trigger: {expected}"

        # No skipped triggers should be in safe list
        for skipped in SKIPPED_TRIGGERS.keys():
            assert skipped not in safe_names, f"Unsafe trigger {skipped} in safe list!"

        print("✓ Subset rules correctly enforced")

    def test_l4_c1_skipped_triggers_with_reasons(self, pg_adapter):
        """
        C1.2: Skipped triggers have documented reasons

        Requirement: get_skipped_triggers() returns triggers with skip_reasons
        """
        print("\n=== C1.2: Skipped triggers with reasons ===")

        skipped = pg_adapter.get_skipped_triggers('public')
        skipped_names = {t['name']: t.get('skip_reasons', []) for t in skipped}

        print(f"  Skipped triggers: {len(skipped)}")
        for name, reasons in skipped_names.items():
            print(f"    ✗ {name}: {reasons}")

        # All expected skipped triggers should be present with correct reasons
        for trig_name, expected_reason in SKIPPED_TRIGGERS.items():
            assert trig_name in skipped_names, f"Expected {trig_name} to be skipped"
            reasons = skipped_names[trig_name]
            assert any(expected_reason in r for r in reasons), \
                f"{trig_name} should have reason containing '{expected_reason}', got {reasons}"

        print("✓ Skipped triggers documented with reasons")


class TestPostgreSQLL4Extraction:
    """L4 Harness: C2 - Extraction tests"""

    @pytest.fixture(scope='class')
    def pg_adapter(self):
        """PostgreSQL adapter fixture"""
        from extensions.plugins.postgresql_adapter import PostgreSQLAdapter

        adapter = PostgreSQLAdapter(
            host='localhost',
            port=5433,
            database='saiql_phase11_test',
            user='saiql_user',
            password='SaiqlTestPass123'
        )
        yield adapter
        adapter.close()

    def test_l4_c2_trigger_enumeration(self, pg_adapter):
        """
        C2.1: Enumerate all triggers in schema

        Requirement: get_triggers() returns list of triggers with required fields
        """
        print("\n=== C2.1: Trigger enumeration ===")

        triggers = pg_adapter.get_triggers('public')
        print(f"  Found {len(triggers)} triggers")

        trig_names = [t['name'] for t in triggers]
        all_expected = SAFE_TRIGGERS + list(SKIPPED_TRIGGERS.keys())

        for expected in all_expected:
            assert expected in trig_names, f"Missing expected trigger: {expected}"

        assert len(triggers) >= len(all_expected), \
            f"Expected at least {len(all_expected)} triggers, got {len(triggers)}"

        print(f"✓ Trigger enumeration successful ({len(triggers)} triggers)")

    def test_l4_c2_trigger_structure(self, pg_adapter):
        """
        C2.2: Trigger structure validation

        Requirement: Each trigger dict has table, timing, events, function_name, level, definition
        """
        print("\n=== C2.2: Trigger structure ===")

        triggers = pg_adapter.get_triggers('public')
        required_fields = ['name', 'schema', 'table_name', 'function_name', 'timing', 'events', 'level', 'definition']

        for trigger in triggers:
            if trigger['name'] in SAFE_TRIGGERS or trigger['name'] in SKIPPED_TRIGGERS:
                for field in required_fields:
                    assert field in trigger, f"Trigger {trigger['name']} missing '{field}'"

                print(f"  ✓ {trigger['name']}: {trigger['timing']} {trigger['events']} "
                      f"on {trigger['table_name']} ({trigger['level']})")

        print("✓ All triggers have required structure")

    def test_l4_c2_trigger_function_retrieval(self, pg_adapter):
        """
        C2.3: Trigger function retrieval

        Requirement: get_trigger_function() returns the associated function
        """
        print("\n=== C2.3: Trigger function retrieval ===")

        for trig_name in SAFE_TRIGGERS:
            func = pg_adapter.get_trigger_function(trig_name, 'public')

            assert func is not None, f"Could not get function for trigger {trig_name}"
            assert 'name' in func, f"Function missing 'name'"
            assert 'definition' in func, f"Function missing 'definition'"

            print(f"  ✓ {trig_name} -> {func['name']}")

        print("✓ Trigger function retrieval successful")


class TestPostgreSQLL4Emission:
    """L4 Harness: C3 - Emission tests"""

    @pytest.fixture(scope='class')
    def pg_adapter(self):
        """PostgreSQL adapter fixture"""
        from extensions.plugins.postgresql_adapter import PostgreSQLAdapter

        adapter = PostgreSQLAdapter(
            host='localhost',
            port=5433,
            database='saiql_phase11_test',
            user='saiql_user',
            password='SaiqlTestPass123'
        )
        yield adapter
        adapter.close()

    def test_l4_c3_single_trigger_creation(self, pg_adapter):
        """
        C3.1: Create single trigger

        Requirement: create_trigger() creates a trigger from definition
        """
        print("\n=== C3.1: Single trigger creation ===")

        # First create the trigger function
        func_def = """
        CREATE OR REPLACE FUNCTION fn_test_l4_harness()
        RETURNS TRIGGER
        LANGUAGE plpgsql
        VOLATILE
        AS $$
        BEGIN
            RETURN NEW;
        END;
        $$
        """
        func_result = pg_adapter.create_function(func_def)
        assert func_result['success'], f"Failed to create function: {func_result.get('error')}"

        # Create trigger
        trig_def = """
        CREATE TRIGGER trg_test_l4_harness
        AFTER INSERT ON departments
        FOR EACH ROW
        EXECUTE FUNCTION fn_test_l4_harness()
        """

        result = pg_adapter.create_trigger(trig_def)
        assert result['success'], f"Failed to create trigger: {result.get('error')}"

        # Verify it exists
        triggers = pg_adapter.get_triggers('public')
        trig_names = [t['name'] for t in triggers]
        assert 'trg_test_l4_harness' in trig_names, "Trigger not found after creation"

        # Cleanup
        pg_adapter.drop_trigger('trg_test_l4_harness', 'departments')
        pg_adapter.drop_function('fn_test_l4_harness')

        print("✓ Single trigger creation successful")

    def test_l4_c3_trigger_recreation(self, pg_adapter):
        """
        C3.2: Trigger recreation (extract -> drop -> recreate)

        Requirement: Extract trigger, drop it, recreate from extracted definition
        """
        print("\n=== C3.2: Trigger recreation ===")

        test_trigger = 'trg_salary_validate'

        # Get trigger info
        triggers = pg_adapter.get_triggers('public')
        trigger = next((t for t in triggers if t['name'] == test_trigger), None)
        assert trigger is not None, f"Could not find trigger {test_trigger}"

        original_def = trigger['definition']
        print(f"  Original definition: {original_def[:50]}...")

        # Get associated function
        func = pg_adapter.get_trigger_function(test_trigger, 'public')
        assert func is not None, "Could not get trigger function"

        # Drop trigger (function stays)
        drop_result = pg_adapter.drop_trigger(test_trigger, trigger['table_name'])
        assert drop_result['success'], f"Failed to drop trigger: {drop_result.get('error')}"

        # Verify it's gone
        triggers = pg_adapter.get_triggers('public')
        trig_names = [t['name'] for t in triggers]
        assert test_trigger not in trig_names, f"Trigger {test_trigger} should be dropped"

        # Recreate trigger (function still exists)
        create_result = pg_adapter.create_trigger(original_def)
        assert create_result['success'], f"Failed to recreate trigger: {create_result.get('error')}"

        # Verify it exists again
        triggers = pg_adapter.get_triggers('public')
        trig_names = [t['name'] for t in triggers]
        assert test_trigger in trig_names, "Trigger not found after recreation"

        print("✓ Trigger recreation successful")


class TestPostgreSQLL4Validation:
    """L4 Harness: C4 - Behavioral validation tests"""

    @pytest.fixture(scope='class')
    def pg_adapter(self):
        """PostgreSQL adapter fixture"""
        from extensions.plugins.postgresql_adapter import PostgreSQLAdapter

        adapter = PostgreSQLAdapter(
            host='localhost',
            port=5433,
            database='saiql_phase11_test',
            user='saiql_user',
            password='SaiqlTestPass123'
        )
        yield adapter
        adapter.close()

    def test_l4_c4_timestamp_trigger_effect(self, pg_adapter):
        """
        C4.1: Validate timestamp trigger effect

        Requirement: trg_employee_update_timestamp sets last_modified on INSERT/UPDATE
        """
        print("\n=== C4.1: Timestamp trigger effect ===")

        # Insert a new employee
        result = pg_adapter.execute_query("""
            INSERT INTO employees (email, first_name, last_name, dept_id, hire_date, salary)
            VALUES ('timestamp_test@example.com', 'Timestamp', 'Test', 1, '2024-01-01', 50000.00)
            RETURNING emp_id, last_modified
        """)
        assert result['success'], f"Insert failed: {result.get('error')}"
        emp_id = result['data'][0]['emp_id']
        last_modified = result['data'][0]['last_modified']

        assert last_modified is not None, "Trigger should set last_modified on INSERT"
        print(f"  ✓ INSERT set last_modified: {last_modified}")

        # Update the employee
        first_modified = last_modified
        result = pg_adapter.execute_query(f"""
            UPDATE employees SET first_name = 'Updated'
            WHERE emp_id = {emp_id}
            RETURNING last_modified
        """)
        assert result['success'], f"Update failed: {result.get('error')}"
        new_modified = result['data'][0]['last_modified']

        assert new_modified is not None, "Trigger should set last_modified on UPDATE"
        assert new_modified >= first_modified, "last_modified should be updated"
        print(f"  ✓ UPDATE updated last_modified: {new_modified}")

        # Cleanup
        pg_adapter.execute_query(f"DELETE FROM employees WHERE emp_id = {emp_id}")

        print("✓ Timestamp trigger effect validated")

    def test_l4_c4_validation_trigger_effect(self, pg_adapter):
        """
        C4.2: Validate salary validation trigger effect

        Requirement: trg_salary_validate rejects negative salary
        """
        print("\n=== C4.2: Validation trigger effect ===")

        # Try to insert employee with negative salary (should fail)
        result = pg_adapter.execute_query("""
            INSERT INTO employees (email, first_name, last_name, dept_id, hire_date, salary)
            VALUES ('negative_salary@example.com', 'Negative', 'Salary', 1, '2024-01-01', -1000.00)
        """)

        assert not result['success'], "Negative salary insert should fail"
        assert 'cannot be negative' in result.get('error', '').lower() or 'negative' in result.get('error', '').lower(), \
            f"Expected 'negative' in error message, got: {result.get('error')}"

        print(f"  ✓ Negative salary rejected: {result.get('error')[:50]}...")

        # Insert with valid salary should succeed
        result = pg_adapter.execute_query("""
            INSERT INTO employees (email, first_name, last_name, dept_id, hire_date, salary)
            VALUES ('valid_salary@example.com', 'Valid', 'Salary', 1, '2024-01-01', 50000.00)
            RETURNING emp_id
        """)
        assert result['success'], f"Valid salary insert should succeed: {result.get('error')}"
        emp_id = result['data'][0]['emp_id']
        print(f"  ✓ Valid salary accepted: emp_id={emp_id}")

        # Cleanup
        pg_adapter.execute_query(f"DELETE FROM employees WHERE emp_id = {emp_id}")

        print("✓ Validation trigger effect validated")

    def test_l4_c4_uppercase_trigger_effect(self, pg_adapter):
        """
        C4.3: Validate uppercase trigger effect

        Requirement: trg_dept_code_uppercase converts dept_code to uppercase
        """
        print("\n=== C4.3: Uppercase trigger effect ===")

        # Insert department with lowercase code
        result = pg_adapter.execute_query("""
            INSERT INTO departments (dept_id, dept_name, dept_code)
            VALUES (99, 'Test Department', 'test')
            RETURNING dept_code
        """)
        assert result['success'], f"Insert failed: {result.get('error')}"
        dept_code = result['data'][0]['dept_code']

        assert dept_code == 'TEST', f"Trigger should uppercase dept_code, got '{dept_code}'"
        print(f"  ✓ INSERT: 'test' -> '{dept_code}'")

        # Update with lowercase
        result = pg_adapter.execute_query("""
            UPDATE departments SET dept_code = 'updt'
            WHERE dept_id = 99
            RETURNING dept_code
        """)
        assert result['success'], f"Update failed: {result.get('error')}"
        new_code = result['data'][0]['dept_code']

        assert new_code == 'UPDT', f"Trigger should uppercase on UPDATE, got '{new_code}'"
        print(f"  ✓ UPDATE: 'updt' -> '{new_code}'")

        # Cleanup
        pg_adapter.execute_query("DELETE FROM departments WHERE dept_id = 99")

        print("✓ Uppercase trigger effect validated")

    def test_l4_c4_null_salary_allowed(self, pg_adapter):
        """
        C4.4: Validate NULL salary is allowed

        Requirement: trg_salary_validate allows NULL salary (only rejects negative)
        """
        print("\n=== C4.4: NULL salary validation ===")

        # Insert with NULL salary (should succeed)
        result = pg_adapter.execute_query("""
            INSERT INTO employees (email, first_name, last_name, dept_id, hire_date, salary)
            VALUES ('null_salary@example.com', 'Null', 'Salary', 1, '2024-01-01', NULL)
            RETURNING emp_id, salary
        """)
        assert result['success'], f"NULL salary insert should succeed: {result.get('error')}"
        emp_id = result['data'][0]['emp_id']
        salary = result['data'][0]['salary']

        assert salary is None, f"Salary should be NULL, got {salary}"
        print(f"  ✓ NULL salary accepted: emp_id={emp_id}")

        # Cleanup
        pg_adapter.execute_query(f"DELETE FROM employees WHERE emp_id = {emp_id}")

        print("✓ NULL salary validation passed")


class TestPostgreSQLL4Limitations:
    """L4 Harness: C5 - Limitations tests"""

    @pytest.fixture(scope='class')
    def pg_adapter(self):
        """PostgreSQL adapter fixture"""
        from extensions.plugins.postgresql_adapter import PostgreSQLAdapter

        adapter = PostgreSQLAdapter(
            host='localhost',
            port=5433,
            database='saiql_phase11_test',
            user='saiql_user',
            password='SaiqlTestPass123'
        )
        yield adapter
        adapter.close()

    def test_l4_c5_limitations_counted(self, pg_adapter):
        """
        C5.1: Skipped triggers listed with count

        Requirement: Limitations report includes count of skipped triggers
        """
        print("\n=== C5.1: Limitations count ===")

        all_triggers = pg_adapter.get_triggers('public')
        safe_triggers = pg_adapter.get_safe_triggers('public')
        skipped = pg_adapter.get_skipped_triggers('public')

        # Filter to only our test triggers
        test_trig_names = set(SAFE_TRIGGERS) | set(SKIPPED_TRIGGERS.keys())
        test_all = [t for t in all_triggers if t['name'] in test_trig_names]
        test_safe = [t for t in safe_triggers if t['name'] in test_trig_names]
        test_skipped = [t for t in skipped if t['name'] in test_trig_names]

        print(f"  Total test triggers: {len(test_all)}")
        print(f"  Safe (migrated): {len(test_safe)}")
        print(f"  Skipped: {len(test_skipped)}")

        assert len(test_safe) == len(SAFE_TRIGGERS), \
            f"Expected {len(SAFE_TRIGGERS)} safe, got {len(test_safe)}"
        assert len(test_skipped) == len(SKIPPED_TRIGGERS), \
            f"Expected {len(SKIPPED_TRIGGERS)} skipped, got {len(test_skipped)}"

        print("✓ Limitations correctly counted")

    def test_l4_c5_limitations_documented(self, pg_adapter):
        """
        C5.2: Limitations documented

        Requirement: Any trigger not supported must be listed with reason
        """
        print("\n=== C5.2: Limitations documented ===")

        # Current L4 limitations
        limitations = [
            "STATEMENT-level triggers: Not portable across databases, skipped",
            "INSTEAD OF triggers: View-specific, require special handling",
            "Constraint triggers: Deferred constraint handling not portable",
            "Event triggers: PostgreSQL-specific DDL event triggers",
            "Triggers modifying other tables: Side effects may cause issues"
        ]

        print("  Documented limitations:")
        for lim in limitations:
            print(f"    - {lim}")

        print("✓ Limitations documented")


class TestPostgreSQLL4Requirements:
    """L4 Harness: Verify all L4 requirements met"""

    @pytest.fixture(scope='class')
    def pg_adapter(self):
        """PostgreSQL adapter fixture"""
        from extensions.plugins.postgresql_adapter import PostgreSQLAdapter

        adapter = PostgreSQLAdapter(
            host='localhost',
            port=5433,
            database='saiql_phase11_test',
            user='saiql_user',
            password='SaiqlTestPass123'
        )
        yield adapter
        adapter.close()

    def test_all_l4_methods_exist(self, pg_adapter):
        """
        Verify adapter implements all L4 methods

        Requirements:
        - get_triggers() exists
        - get_trigger_function() exists
        - get_safe_triggers() exists
        - get_skipped_triggers() exists
        - create_trigger() exists
        - drop_trigger() exists
        - create_triggers_in_order() exists
        """
        print("\n=== Verifying all L4 requirements ===")

        required_methods = [
            'get_triggers',
            'get_trigger_function',
            'get_safe_triggers',
            'get_skipped_triggers',
            'create_trigger',
            'drop_trigger',
            'create_triggers_in_order'
        ]

        for method in required_methods:
            assert hasattr(pg_adapter, method), f"Missing {method}() method"
            assert callable(getattr(pg_adapter, method)), f"{method}() not callable"
            print(f"  ✓ {method}()")

        print("✓ All L4 methods exist")
